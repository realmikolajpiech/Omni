...
def classify_files_endpoint():
    ...
    prompt = (
        ...
        "1. BINARIES & MODELS: .exe, .dll, .so, .dylib, .bin, .pkl, .pth, .tflite, .onnx, .wasm, .hprof, .blend, .jks, .keystore\n"
        "2. MEDIA: Images (.png, .jpg), Audio (.mp3, .wav), Video (.mp4), Fonts (.ttf, .otf) — these are indexed separately.\n"
        ...
        "5. CONFIG METADATA: .pbxproj, .xcworkspacedata, .plist, .storyboard, .xib, .entitlements, .idea/, .vscode/\n"
        ...
        "8. BOILERPLATE: .eslintrc, .prettierrc, tsconfig.json, .gitignore, manifest.json, license.txt\n"
        "9. UNITY/GAME ASSETS: .meta, .prefab, .unity, .mat, .asset\n\n"
        "RULES - Output 1 (INDEX) for:\n"
        "1. SOURCE CODE: .py, .js, .ts, .rs, .go, .c, .cpp, .java, .swift (containing LOGIC)\n"
        "2. DOCUMENTS: .md, .txt (notes), .docx, .pdf, .rtf\n"
        "3. CONFIG WITH LOGIC: specific .py/.js config files that contain custom logic (not just key-value pairs)\n\n"
        "EXAMPLES:\n"
